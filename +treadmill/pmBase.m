...
switch state
    ...
    case p.trial.pldaps.trialStates.trialSetup
        ...
        p.trial.(sn).state              = p.trial.(sn).states.STIMULUS;
...
%% texPrepare
    ...
    function texPrepare(p, sn)
        % generate grating % NEED TO MAKE THIS HAPPEN ONCE PER TRIAL
        p.trial.(sn).graTex = CreateProceduralSineGrating(p.trial.display.ptr, p.trial.display.screenSize(3), p.trial.display.screenSize(3), [], [], [] );
