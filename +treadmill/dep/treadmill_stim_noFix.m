function p = treadmill_stim_noFix(subj,inputType,viewdist,grbl_command) 
...
p.defaultParameters.pldaps.finish = 60;
...
%% Load in a big honkin' 4D array of all the images

% hardcoding this for now. Fix later

disp('Not frozen, just loading a lot of files...');
cd('/home/huklab/Documents/3FaceBars0Deg20200109');
clear allShots;
load('allShots.mat');
mergedShots(:,:,:,1)= allShots;

cd('/home/huklab/Documents/3FaceBars45Deg20200109');
clear allShots;
load('allShots.mat');
mergedShots(:,:,:,2)= allShots;

cd('/home/huklab/Documents/3FaceBars90Deg20200109');
clear allShots;
load('allShots.mat');
mergedShots(:,:,:,3)= allShots;

cd('/home/huklab/Documents/3FaceBars135Deg20200109');
clear allShots;
load('allShots.mat');
mergedShots(:,:,:,4)= allShots;

allShots = mergedShots;

global allShots;

%an ATU is the distance in cm between frames
p.trial.treadmill.ATU = 300/length(allShots); %300 is the length of the hallway in cm 
p.trial.treadmill.scaleFactor = 0.00942; %linear length in cm of one encoder tick
p.trial.treadmill.endPoint = size(allShots, 3);
cd('/home/huklab/MLtoolbox/huklabBasics');
clear mergedShots;
disp('Done!');

%% Generate random grating direction for each trial in this block
for i = 1:p.defaultParameters.pldaps.finish
    p.trial.treadmill.grateDir(i) = randi([1 size(allShots, 4)]);
end

%% clear the screen so you don't waste 60s testing every change
sca;
